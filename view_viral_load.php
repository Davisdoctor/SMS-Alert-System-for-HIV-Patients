                        ...
                        <section class="with-table">
                            <table class="datatable tablesort selectable paginate full">
                                ...
                                <tbody>
                                    ...
                                    $x = 1;
                                    include 'init.php';
                                    $query = mysql_query("select * from viral_load order by Viral_Load_Id desc");
                                    while ($row = mysql_fetch_assoc($query)) {
                                        ?>
                                        <tr>
                                            <td><?php echo get_full_name($row['Patient_Id'], "Patient_Id", "First_Name", "Last_name", "patients"); ?></td>
                                            <td><?php echo $row['VL_Numbers'] ?></td>
                                             <td><?php echo streamline_date($row['Date_Sample_Drawn']); ?></td>
                                            <td><?php echo streamline_date($row['Date_Sample_Returned']); ?></td>
                                        </tr>
                                        <?php
                                        $x++;
                                    }
